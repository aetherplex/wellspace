  ...
  <Layout>
    <div>
      <div>
        ...
        >{{ $page.gcms.course.tag }}</p>
      </div>
      <h1 class="my-3 text-3xl">{{ $page.gcms.course.title }}</h1>
      <p class="mb-6">
        Taught by
        <strong>{{ $page.gcms.course.instructor }}</strong>
      </p>
      <div class="flex justify-start items-center">
        <Timer />
        <p
          class="ml-2 text-left text-xs text-blue-500 py-2"
        >{{ $page.gcms.course.timeToComplete }} hours to complete.</p>
      </div>
      <hr class="mt-4 mb-8" />
      <g-image :src="$page.gcms.course.image.url" class="rounded-sm" />
      <article class="mt-8">
        <strong>{{ $page.gcms.course.description }}</strong>
        <br />
        <br />Lorem ipsum dolor sit amet consectetur adipisicing elit. Minus dolorem
